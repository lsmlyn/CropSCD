...
# from models.internimage.intern_image import InternImage
...
class ShiftWindowMSA(nn.Module):
    ...
    def windowpartition(self, x):
        ...
        windows = windows.view(-1, window_size, window_size, C)     #B*num_window, window_size, window_size, C
        return windows
...
class SwinTransformer(nn.Module):
    ...
    '''initialize backbone'''
    def initweights(self, swin_type='swin_tiny_patch4_window7_224', pretrained_model_path='', custom_weight=False):
        # load
        ...
        conv1_weight = state_dict["patch_embed.projection.weight"]
        ...
        state_dict["patch_embed.projection.weight"] = conv1_weight
        # load state_dict
...
def Build_Backbone(
    backbone_name="Swin_tiny_p4w7", pretrained=True, band_num=4, pretrained_path='', **kwargs
):
    if 'Swin' in backbone_name:
        if "Swin_tiny_p4w7" in backbone_name:
            ...
            if pretrained:
                pretrained_path = os.path.join(pretrained_path, 'swin/swin_base_patch4_window7_224_22k.pth')
                net.initweights('swin_base_patch4_window7_224', pretrained_path)
            return net, [768, 384, 192, 96], [True, True, True, True]
        elif "Swin_large_p4w12" in backbone_name:
    ...
    elif "Intern" in backbone_name:
        if "Internimage_T" in backbone_name:
            net = InternImage(in_channels=band_num, depths= [4, 4, 18, 4],groups=[4, 8, 16, 32], channels=64)
            if pretrained:
                pretrained_path = os.path.join(pretrained_path, 'intern/internimage_t_1k_224.pth')
                net.load_pretrained(pretrained_path)
            return net, [512, 256, 128, 64], [True, True, True, True]
        elif "Internimage_S" in backbone_name:
            net = InternImage(in_channels=band_num, depths= [4, 4, 21, 4],groups=[5, 10, 20, 40], channels=80)
            if pretrained:
                pretrained_path = os.path.join(pretrained_path, 'intern/internimage_s_1k_224.pth')
                net.load_pretrained(pretrained_path)
            return net, [640, 320, 160, 80], [True, True, True, True]
        elif "Internimage_B" in backbone_name:
            net = InternImage(in_channels=band_num, depths=[4, 4, 21, 4],groups=[7, 14, 28, 56], channels=112)
            if pretrained:
                pretrained_path = os.path.join(pretrained_path, 'intern/internimage_b_1k_224.pth')
                net.load_pretrained(pretrained_path)
            return net, [896, 448, 224, 112], [True, True, True, True]
        elif "Internimage_L" in backbone_name:
            net = InternImage(in_channels=band_num, depths= [5, 5, 22, 5],groups= [10, 20, 40, 80], channels=160)
            if pretrained:
                pretrained_path = os.path.join(pretrained_path, 'intern/internimage_l_22kto1k_384.pth')
                net.load_pretrained(pretrained_path)
            return net, [1280, 640, 320, 160], [True, True, True, True]
        elif "Internimage_XL" in backbone_name:
            net = InternImage(in_channels=band_num, depths= [5, 5, 24, 5],groups= [12, 24, 48, 96],channels=192)
            if pretrained:
                pretrained_path = os.path.join(pretrained_path, 'intern/internimage_xl_22kto1k_384.pth')
                net.load_pretrained(pretrained_path)
            return net,  [1280, 640, 320, 160], [True, True, True, True]
    else:
        raise ValueError(
