...
#                   query_update[i] = torch.sum(query[idx].squeeze(1), dim=0)
    ...
    def get_score(self, mem, query):
        ...
        query.to(device=torch.device('cuda'))
        mem.to(device=torch.device('cuda'))

        score = torch.matmul(query, torch.t(mem).to(device=torch.device('cuda')))# b X h X w X m
    ...
    def update(self, query, keys,train):
        ...
        if train:
            ...
            updated_memory = F.normalize(query_update.to(device=torch.device('cuda')) + keys.to(device=torch.device('cuda')), dim=1)
    ...
    def spread_loss(self,query, keys, train):
        ...
        spreading_loss = loss(query_reshape.to(device=torch.device('cuda')), pos.detach().to(device=torch.device('cuda')), neg.detach().to(device=torch.device('cuda')))
    ...
    def gather_loss(self, query, keys, train):
        ...
        gathering_loss = loss_mse(query_reshape.to(device=torch.device('cuda')), keys[gathering_indices].squeeze(1).detach().to(device=torch.device('cuda')))
    ...
    def read(self, query, updated_memory):
        ...
        concat_memory = torch.matmul(softmax_score_memory.detach().to(device=torch.device('cuda')), updated_memory.to(device=torch.device('cuda'))) # (b X h X w) X d
